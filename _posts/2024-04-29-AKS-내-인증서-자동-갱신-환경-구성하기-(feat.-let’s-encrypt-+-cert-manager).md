...
export AZURE_LOADBALANCER_DNS_LABEL_NAME=<AKS LB의 DNS Label> # EX) 다음과 같이 임의 지정 = demo-lb
export AKS_OIDC_ISSUER="$(az aks show -n $CLUSTER -g $AZURE_DEFAULTS_GROUP --query "oidcIssuerProfile.issuerUrl" -o tsv)" # 생략 가능할듯한데
...
# 정상 설치 여부 및 컴포넌트 확인
kubectl -n cert-manager get all # STATUS Running 확인
...
{% raw %}
```bash
# AKS 클러스터의 Managed Identity(이하 MI) 인증 활성
